...
Добавьте приватное свойство для хранения дня рождения пользователя (в формате мм-дд-гггг) и
 реализуйте геттер с именем birthday для его чтения.
...
Для каждого из сотрудников по 2-3 раза добавьте через метод addDays рабочие дни (попробуйте разные
 вариации, к примеру, добавить -2 дня или 50 дней)
Выведите зарплату за текущий месяц для каждого из работников в консоль в формате «Имя Фамилия - N
рублей». Для вывода имен используйте метод getFullName.
...
Обратите внимание, что в дочернем классе в конструкторе вам необходимо использовать вызов родительского
 конструктора через метод super() для того, чтобы вы могли использовать this и обращение к свойствам
